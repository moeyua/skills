...
These are generation results. Retain the actual managed projection for the audit baseline and continue to Plan's [audit and corrections](audit.md) before the final report, including after a partial result. Keep the generation result distinct from the later audit and correction results.

## Audit correction boundary

Plan may correct the authorized local plan even when the remote portion failed. An Issue correction requires a definitively completed remote generation transaction with no failure, conflict or ambiguity stop condition. Reconciliation of an ambiguous edit does not authorize a later corrective edit. Preserve the original result and report unresolved remote findings instead.

Correct and validate the local plan first. If the problem projection is unchanged, do not edit the Issue. Otherwise reread the canonical Issue, require the same bounded problem and a valid managed envelope under the existing ownership rules, and additionally require its managed title/type/body projection to match the audited baseline. A valid digest for a different projection is not permission to overwrite that change. Preserve the fresh read's outside body byte-for-byte and leave comments, unrelated labels and all project-management fields untouched.

Apply at most one additional corrective edit using the shared remote correction transaction, including label preflight, temporary-file cleanup and one read-back. This is separate from the generation edit, not a retry or new Issue identity. Report generation, audit and correction outcomes separately, with all labels created across both phases. Corrections do not grant adoption or edits to human-owned content.
