...
An Issue failure makes the overall result `partial`; it never removes, invalidates, or blocks implementation from the completed local plan. Do not fall back, retry automatically, or create a replacement Issue after an ambiguous result.
...
3. Ground both artifacts from the same settled intent. Render their target-specific structure without letting either artifact introduce new scope.
...
9. Create exactly one Issue from a safe temporary body file. Capture its canonical URL and remove the temporary file.
