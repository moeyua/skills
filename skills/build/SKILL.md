---
name: build
description: 'Execute an approved plan into code that fits the project''s style; run TDD when the project has tests. Use when the user says "implement" / "build it" / "apply the plan" / "实现" / "落实" / "按方案做", or right after plan produces a plan. Not for coding before a plan exists (run plan first), writing prose docs, or changing code outside the plan.'
...
Build turns an approved plan into code that fits the project. The intent work is already done in plan; here you only execute — no redesigning, no drive-by fixes, no drifting from the plan. Every rule below exists to stop you from reopening intent decisions mid-execution.

No plan yet? Run `/plan` first.
...
1. **Locate the plan**: if the user's message has a path, use it; otherwise take the newest `status: approved` plan under `plans/` (sorted by YYYY-MM-DD). If there is none, report the state and ask the user to point to one or run `/plan` first — don't guess the goal.
...
- **No placeholders** (`TBD` / `TODO` / `implement later` / `similar to step N`) — these signal an unfinished plan; go back to plan to complete it.
...
- **The files and interfaces the plan assumes still exist** — grep the paths and function names the plan names. Drift means the plan is out of sync with the code; go back to plan to fix the plan.
...
This keeps the work off the mainline so the later commit and PR have a branch to live on — pull-request refuses to push a protected branch, so landing the work there just forces a detour later. Building on a clean working tree (Preflight already required it) means `git checkout -b` carries nothing unexpected across. Report which branch you're on before editing.
...
- **A failure that reflects a real bug is not build's to patch here** — route to `/plan fix`.
...
These all share one root: don't re-judge intent while implementing. Intent was judged in plan; reopening it here is where scope creep starts.
...
- **Don't touch dependencies**: unless the plan says so, add no new dependency, bump nothing up or down, leave the lockfile alone. Adding a dependency is a product decision and belongs to plan.
