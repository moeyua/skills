...
**PRODUCT is special** — document does not author its content (it has no format file); a change to philosophy/boundaries is plan's job. Document may at most create an empty skeleton and route back to `/plan`.

## Record: merge a spec delta

Plan writes a `## Spec delta` into the plan when a change alters behavior worth documenting. Merging is mechanical, by requirement name:
...
- **vs plan** — the plan is _this change's how_ (ephemeral, archived); documentation is _the project's what_ (persistent, maintained). Don't restate implementation steps into durable memory.
- **vs build** — build writes code and tests; document writes docs. Document never touches code or runs git.
- **vs verify** — verify checks a change holds up; document records what the change established. Document doesn't judge correctness.
- **vs health** — health detects drift/gaps across the whole project (read-only); document writes the correction. Document acts on awareness — from health or a person — it never owns detection.
...
Document's failure mode is writing truth that wasn't earned — guessed, over-scoped, or copied from implementation. Stop and report in these cases:
...
- **The target is PRODUCT's content** — that's philosophy work; route to `/plan`.
- **You'd need to reshape intent, not just document it** — that's plan work; route back to `/plan`.
