...
Health is a project checkup — it audits whether a project's memory still holds up against its code, then reports what has drifted, and stops there. It is the orthogonal-audit half of the verify pillar: verify gates one change before merge; health steps back and looks at the whole project, outside the loop. Every rule here exists to keep health's word **trustworthy and advisory**: it reports what it found and points you at the skill that fixes it — it never edits, commits, or chains onward itself.
...
**Health confirms drift, it doesn't fix it.** It writes no file, commits nothing, calls no other skill to act. It runs read-only observation commands (the bundled checker, `pnpm outdated`, `gh run list`, `git log`) — observing is not editing. The moment health touches a file, the author loses the chance to decide.
