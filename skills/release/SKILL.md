...
Never guess an ambiguous release identity or repository metadata contract, invent a repository changelog where none exists, mutate before required confirmation, force-push, rewrite history, move/delete a tag or Release, create a PR, deploy, roll back, upload artifacts, or publish a registry package. A verified tag or GitHub Release attests exact release state only; it does not supply missing implementation acceptance or a Verify pass. Ordinary upstream results require no added attestation fields. When formal acceptance is relevant, preserve the actual Verify source, basis and exact verdict + acceptance-field pair, including basis changes or conflicting later Review/Verify evidence that prevents a current acceptance claim. Preserve historical Check records with their original source and time scope; never relabel them as Verify evidence.
