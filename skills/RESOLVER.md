...
| Check     | obtaining a read-only verdict on a change or planning artifacts          | `skills/check/SKILL.md`     |
...
- Explore maps facts; Doctor judges project-wide drift; Check judges a scoped change or planning artifact.
...
Plan's route preserves the explicit `local`, `issue`, and `both` targets; omitted target means `both`. Every invocation audits its readable outputs once through Check in an independent context, then Plan revises clear findings within its artifact authorization and verifies those findings. Missing Check, independent context, or required evidence yields `inconclusive`; the original generation result, audit verdict, and revision result remain separate. This support call leaves new plans at `draft` and does not require Shape or formal implementation acceptance. Ordinary Check does not inherit a universal separate-agent requirement.
