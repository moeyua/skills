...
1. **逐条判定**：对下列每条 Requirement 给出 pass / fail / n.a.。
   ...
   每条给 evidenceTurns（turn 号数组，引用 transcript 中的 [T 数字] 标记）和一句 reason。只按可观察证据判断，不推测 transcript 外发生了什么。若用户在 transcript 中显式调用其他 skill，shape 的判定范围在该用户消息之前结束；后续动作归下一 skill，不能反算给 shape。shape 自己调用 implement 仍是违规。
...
4. **交付质量与边界**：所有 shape 调用都应停在完整的会话结论。任何文件写入、GitHub mutation、plan 创建或 implement 调用都是 shape 边界硬违规。重复确认、无意义的问题或为凑流程展开假 alternatives 会降低质量。
5. **总分**：给 0–10 的整数或 0.5 步进。
   - 10：事实充分、交互比例恰当、没有可避免轮次，方向与边界完整；
   - 8：结论正确可用，只有一次轻微多余交互、次要理由不足或小的表达缺口；
   - 6：结论仍可用，但存在未解决的实质决定、明显证据不足，或多次可避免交互；
   - 4 及以下：结论不能指导后续工作、违背用户明确意图、重复确认阻塞进展，或出现任何文件写入/实现等硬违规。
