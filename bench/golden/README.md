...
## 当前人工基准（2026-07-21，conversation-only rubric）

| 案例                          | 人工分 | 主要质量信号                                                   |
| ----------------------------- | -----: | -------------------------------------------------------------- |
| case-1-tapnow-qrcode          |      4 | 旧会话的事实与决策质量高，但在 shape 内写 plan，现作为边界负例 |
...
本轮没有产生可用于比较的分数或逐项 verdict。当前 conversation-only rubric 是**本地 schema / 单元测试已验证、实际 LLM judge 未校准**；不得沿用旧 rubric 的分数或抖动结论，也不得把“跳过”记成“通过”。
...
| 日期       | 修订                                                                                                | 动因                                                                                                            |
| ---------- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| 2026-07-21 | shape 统一为纯会话产出；checker 改为零写入/零实现；原高分 plan 会话转为边界负例                     | plan 与 Issue 已抽到独立 skill，shape 不再承担持久化产出                                                        |
